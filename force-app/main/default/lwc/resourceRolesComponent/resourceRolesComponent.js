...
export default class ResourceRolesComponent extends LightningElement {
    ...
    @track newRole = { name: '', billingUnit: 'Hour', price: null, cost: null, location: '' };
    columns = columns;
    wiredRolesResult;
    ...
    handleNewResource() {
        this.newRole = { name: '', billingUnit: 'Hour', price: null, cost: null, location: '' };
    ...
    handleSaveNewRole() {
        if (!this.newRole.name) {
            this.dispatchEvent(new ShowToastEvent({ title: 'Error', message: 'Role Name is required', variant: 'error' }));
            return;
        }
        createResourceRole({
            ...
            .then(() => {
                this.dispatchEvent(new ShowToastEvent({ title: 'Success', message: 'Resource Role created', variant: 'success' }));
                this.isModalOpen = false;
                return refreshApex(this.wiredRolesResult);
            })
            .catch(error => {
                this.dispatchEvent(new ShowToastEvent({ title: 'Error', message: error.body?.message || 'Error creating role', variant: 'error' }));
            });
    }
    ...
    handleSave(event) {
        ...
        updateResourceRoles({ roles: records })
            .then(() => {
                this.dispatchEvent(new ShowToastEvent({ title: 'Success', message: 'Resource Roles updated', variant: 'success' }));
                this.draftValues = [];
                return refreshApex(this.wiredRolesResult);
            })
            .catch(error => {
                this.dispatchEvent(new ShowToastEvent({ title: 'Error', message: error.body?.message || 'Error updating roles', variant: 'error' }));
            });
    }
        ...
        switch (actionName) {
            ...
            case 'delete':
                deleteResourceRole({ roleId: row.Id })
                    .then(() => {
                        this.dispatchEvent(new ShowToastEvent({ title: 'Success', message: 'Resource Role deleted', variant: 'success' }));
                        return refreshApex(this.wiredRolesResult);
                    })
