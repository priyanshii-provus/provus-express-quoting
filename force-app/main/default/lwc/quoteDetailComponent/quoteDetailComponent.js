...
export default class QuoteDetailComponent extends NavigationMixin(LightningElement) {
    @api recordId;
    ...
    get summaryTabClass()     { return this._tabCls('summary'); }
    get lineItemsTabClass()   { return this._tabCls('line-items'); }
    get timelineTabClass()    { return this._tabCls('timeline'); }
    get generatedPDFsTabClass(){ return this._tabCls('generated-pdfs'); }

    _tabCls(id) { return this.activeTab === id ? 'tab-btn active' : 'tab-btn'; }
    ...
    get formattedStartDate() {
        return this.quote.StartDate
            ? new Date(this.quote.StartDate + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
            : '—';
    }
    get formattedEndDate() {
        return this.quote.EndDate
            ? new Date(this.quote.EndDate + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
            : '—';
    }
    get formattedMargin() {
    ...
    get formattedCreatedDate() {
        return this.quote.CreatedDate
            ? new Date(this.quote.CreatedDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
            : '';
    }
    ...
    /* ───── Approval Getters ───── */
    get isDraft() { return this.quote.Status === 'Draft'; }
    get isPendingApproval() { return this.quote.Status === 'Pending Approval'; }
    ...
    get modalDescription() {
        if (this.approvalAction === 'Submit') return 'Submit quote "' + this.quote.QuoteNumber + '" for approval by Provus Express Quoting Users.';
        if (this.approvalAction === 'Approve') return 'Are you sure you want to approve this quote?';
        if (this.approvalAction === 'Reject') return 'Please provide a reason for rejecting this quote.';
        return '';
    }
    ...
    get isRejectAction() { return this.approvalAction === 'Reject'; }

    /* ───── Revenue Card Computations ───── */
    get laborItems() { return this.lineItems.filter(i => i.Item_Type__c === 'Labor'); }
    get productItems() { return this.lineItems.filter(i => i.Item_Type__c === 'Product'); }
    get addonItems() { return this.lineItems.filter(i => i.Item_Type__c === 'Add-on'); }

    get laborCost()    { return this.sumField(this.laborItems, 'Cost__c'); }
    get laborMargin()  { return this.sumField(this.laborItems, 'Margin__c'); }
    get laborMarginPct() { return this.pct(this.laborMargin, this.quote.LaborRevenue); }
    get laborCount()   { return this.laborItems.length; }

    get productsCost()   { return this.sumField(this.productItems, 'Cost__c'); }
    get productsMargin() { return this.sumField(this.productItems, 'Margin__c'); }
    get productsMarginPct() { return this.pct(this.productsMargin, this.quote.ProductsRevenue); }
    get productsCount()  { return this.productItems.length; }

    get addonsCost()   { return this.sumField(this.addonItems, 'Cost__c'); }
    get addonsMargin() { return this.sumField(this.addonItems, 'Margin__c'); }
    get addonsMarginPct() { return this.pct(this.addonsMargin, this.quote.AddonsRevenue); }
    get addonsCount()  { return this.addonItems.length; }

    /* ───── Chart Data ───── */
    get chartMaxValue() {
        return Math.max(
            this.laborCost, this.laborMargin,
            this.productsCost, this.productsMargin,
            this.addonsCost, this.addonsMargin,
            1
        );
    }
    get laborCostBarPct()     { return this.barPct(this.laborCost); }
    get laborMarginBarPct()   { return this.barPct(this.laborMargin); }
    get productsCostBarPct()  { return this.barPct(this.productsCost); }
    get productsMarginBarPct(){ return this.barPct(this.productsMargin); }
    get addonsCostBarPct()    { return this.barPct(this.addonsCost); }
    get addonsMarginBarPct()  { return this.barPct(this.addonsMargin); }

    barPct(val) { return `height: ${Math.max((val / this.chartMaxValue) * 100, 2)}%`; }
    ...
    get phaseData() {
        ...
        this.lineItems.forEach(item => {
            ...
            if (!phases[phase]) {
                phases[phase] = { phase, labor: 0, products: 0, addons: 0, total: 0, items: 0 };
            }
    ...
    get hasPhaseData() { return this.phaseData.length > 0; }
    ...
    get ganttYears() {
        ...
        months.forEach(m => {
            if (m.year !== currentYear) {
                if (currentYear !== null) years.push({ year: currentYear, colSpan, key: `y-${currentYear}`, style: `grid-column: span ${colSpan};` });
                currentYear = m.year;
                colSpan = 1;
            } else {
                colSpan++;
    ...
    get ganttPhases() {
        ...
        this.lineItems.forEach(item => {
            ...
            if (durationMonths === 0) { durationMonths = 1; durationText = "Under 1 month"; }
            ...
            phasesMap[phase].items.push({
                id: item.Id,
                name: (item.Item_Type__c === 'Labor' || item.Item_Type__c === 'Resource') && item.Resource_Role__r ? item.Resource_Role__r.Name : (item.Product2?.Name || item.Name),
    ...
    get hasTimeline() { return this.ganttMonths.length > 0; }
    
    handleAddMilestone() {
        this.dispatchEvent(new ShowToastEvent({ title: 'Info', message: 'Milestone creation coming soon.', variant: 'info' }));
    }

    /* ───── Helpers ───── */
    sumField(arr, field) { return arr.reduce((s, i) => s + (i[field] || 0), 0); }
    pct(margin, revenue) {
    ...
    handleRefresh() {
        ...
        this.dispatchEvent(new ShowToastEvent({ title: 'Refreshed', message: 'Quote data refreshed', variant: 'success' }));
    }

    handleSave() {
        this.dispatchEvent(new ShowToastEvent({ title: 'Info', message: 'Use inline editing in Line Items tab to save changes', variant: 'info' }));
    }
    ...
    async handleApprovalAction() {
        if (this.isRejectAction && !this.approvalComment) {
            this.dispatchEvent(new ShowToastEvent({ title: 'Error', message: 'Comments are required for rejection', variant: 'error' }));
            return;
        }
        ...
        try {
            if (this.approvalAction === 'Submit') {
                await submitForApproval({ quoteId: this.recordId, comment: this.approvalComment });
                this.dispatchEvent(new ShowToastEvent({ title: 'Success', message: 'Quote submitted for approval', variant: 'success' }));
            } else {
                await processApprovalAction({ quoteId: this.recordId, action: this.approvalAction, comment: this.approvalComment });
                this.dispatchEvent(new ShowToastEvent({ title: 'Success', message: `Quote ${this.approvalAction.toLowerCase()}d`, variant: 'success' }));
            }
            this.closeModal();
            this.handleRefresh();
            this.loadApprovalHistory();
        } catch (error) {
            this.dispatchEvent(new ShowToastEvent({ title: 'Error', message: error.body?.message || 'Action failed', variant: 'error' }));
        } finally {
    ...
    async handleRecall() {
        try {
            await processApprovalAction({ quoteId: this.recordId, action: 'Recall', comment: '' });
            this.dispatchEvent(new ShowToastEvent({ title: 'Success', message: 'Quote approval recalled', variant: 'success' }));
            this.handleRefresh();
            this.loadApprovalHistory();
        } catch (error) {
            this.dispatchEvent(new ShowToastEvent({ title: 'Error', message: error.body?.message || 'Recall failed', variant: 'error' }));
        }
    }
    ...
    loadApprovalHistory() {
        ...
        getApprovalHistory({ quoteId: this.recordId })
            ...
            .catch(error => console.error('Error loading approval history:', error));
    }
    ...
    get hasPDFVersions() { return this.pdfVersions && this.pdfVersions.length > 0; }
    ...
    async handleGeneratePDF() {
        ...
        try {
            ...
            this.dispatchEvent(new ShowToastEvent({ title: 'Success', message: 'PDF version saved', variant: 'success' }));
            this.loadPDFVersions();
            this.activeTab = 'generated-pdfs';
        } catch (error) {
            this.dispatchEvent(new ShowToastEvent({ title: 'Error', message: error.body?.message || 'Failed to generate PDF', variant: 'error' }));
        } finally {
    ...
    async handleDeleteVersion(event) {
        ...
        try {
            ...
            this.dispatchEvent(new ShowToastEvent({ title: 'Success', message: 'PDF version deleted', variant: 'success' }));
            this.loadPDFVersions();
        } catch (error) {
            this.dispatchEvent(new ShowToastEvent({ title: 'Error', message: error.body?.message || 'Failed to delete', variant: 'error' }));
        }
    }
}
