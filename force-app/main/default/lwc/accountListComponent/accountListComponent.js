...
export default class AccountListComponent extends NavigationMixin(LightningElement) {
    @track allAccounts = [];
    ...
    filterAccounts() {
        ...
        if (this.searchKey) {
            filtered = filtered.filter(acc => acc.Name && acc.Name.toLowerCase().includes(this.searchKey));
        }
        ...
        switch (actionName) {
            ...
            case 'delete':
                deleteAccount({ accountId: row.Id })
                    .then(() => {
                        this.dispatchEvent(new ShowToastEvent({ title: 'Success', message: 'Account deleted', variant: 'success' }));
