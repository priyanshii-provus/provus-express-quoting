...
export default class AddonsComponent extends LightningElement {
    ...
    @track newAddon = { name: '', price: null, cost: null, billingUnit: 'Each', description: '' };
    columns = columns;
    wiredAddonsResult;
    ...
    handleNewAddon() {
        this.newAddon = { name: '', price: null, cost: null, billingUnit: 'Each', description: '' };
    ...
    handleSaveNewAddon() {
        if (!this.newAddon.name) {
            this.dispatchEvent(new ShowToastEvent({ title: 'Error', message: 'Add-on Name is required', variant: 'error' }));
            return;
        }
        createAddon({
            ...
            .then(() => {
                this.dispatchEvent(new ShowToastEvent({ title: 'Success', message: 'Add-on created', variant: 'success' }));
                this.isModalOpen = false;
                return refreshApex(this.wiredAddonsResult);
            })
            .catch(error => {
                this.dispatchEvent(new ShowToastEvent({ title: 'Error', message: error.body?.message || 'Error creating add-on', variant: 'error' }));
            });
    }
    ...
    handleSave(event) {
        ...
        updateAddons({ addons: records })
            .then(() => {
                this.dispatchEvent(new ShowToastEvent({ title: 'Success', message: 'Add-ons updated', variant: 'success' }));
                this.draftValues = [];
                return refreshApex(this.wiredAddonsResult);
            })
            .catch(error => {
                this.dispatchEvent(new ShowToastEvent({ title: 'Error', message: error.body?.message || 'Error updating add-ons', variant: 'error' }));
            });
    }
        ...
        switch (actionName) {
            ...
            case 'delete':
                deleteAddon({ addonId: row.Id })
                    .then(() => {
                        this.dispatchEvent(new ShowToastEvent({ title: 'Success', message: 'Add-on deleted', variant: 'success' }));
                        return refreshApex(this.wiredAddonsResult);
                    })
